from scripts.patch_max_canonical_delivery import apply_patch
...
def test_patch_max_canonical_delivery_is_idempotent(tmp_path):
    ...
    assert 'from interfaces.messaging.legacy_bridge import messenger_reply_to_canonical' in first
    assert 'from interfaces.messaging.max.delivery import send_canonical_max_response' in first
    assert "if platform == 'max':" in first
    assert 'send_canonical_max_response(' in first
