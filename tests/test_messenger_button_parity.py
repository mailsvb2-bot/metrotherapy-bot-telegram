...
def test_score_scale_labels_match_platform_score_contracts():
    ...
    vk_keyboard_json = vk_score_scale_keyboard_json()

    assert _vk_keyboard(vk_keyboard_json)["inline"] is False
    assert len(_vk_commands(vk_keyboard_json)) == VK_MAX_TEXT_SCORE_BUTTONS
    assert _vk_labels(vk_keyboard_json) == telegram_expected[:-1] + ["📈 Прогресс", BACK_LABEL]
    assert _max_button_texts(messenger_max_ui.score_scale_attachment()) == numeric_expected + ["📈 Мой прогресс", BACK_LABEL]

    vk_commands = _vk_commands(vk_keyboard_json)
