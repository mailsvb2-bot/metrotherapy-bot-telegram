...
def _install_reward_db(monkeypatch: pytest.MonkeyPatch, conn: sqlite3.Connection) -> None:
    ...
    monkeypatch.setattr(reward_tokens, "utc_now", lambda: datetime(2026, 7, 21, 12, tzinfo=UTC))
...
def test_paid_referral_reward_is_token_backed_and_idempotent(monkeypatch: pytest.MonkeyPatch) -> None:
    conn = _reward_db()
...
def test_referral_limit_is_serialized_on_referrer_wallet(monkeypatch: pytest.MonkeyPatch) -> None:
    conn = _reward_db()
    _install_reward_db(monkeypatch, conn)
...
def test_gift_buyer_reward_is_granted_once(monkeypatch: pytest.MonkeyPatch) -> None:
    conn = _reward_db()
    _install_reward_db(monkeypatch, conn)
    try:
