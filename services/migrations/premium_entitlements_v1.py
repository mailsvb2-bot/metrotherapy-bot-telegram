...
def apply(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_premium_entitlements_user ON premium_entitlements(user_id, entitlement_type, status)"
    )
    conn.execute(
        """
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_premium_delivery_outbox_status ON premium_delivery_outbox(status, id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_premium_delivery_outbox_user ON premium_delivery_outbox(user_id, platform)"
    )
    conn.execute(
        """
