...
def apply(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS practice_wallets(
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS practice_ledger(
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS payment_token_grants(
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS user_practice_preferences(
            user_id INTEGER PRIMARY KEY,
            delivery_mode TEXT NOT NULL DEFAULT 'single_daily',
            updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
        )
        """.strip()
    )
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS practice_reservations(
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_practice_reservations_user_status ON practice_reservations(user_id, status)"
    )
