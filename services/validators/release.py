...
def validate_release_hygiene(*, strict: bool = True) -> None:
    ...
    env_files = [p for p in PROJECT_ROOT.rglob(".env*") if p.is_file() and p.name != ".env.example"]
    runtime_db = [p for p in PROJECT_ROOT.rglob("*") if p.is_file() and (p.suffix in {".db", ".sqlite"} or p.name.endswith((".db-wal", ".db-shm", ".db-journal")))]
    logs = [p for p in PROJECT_ROOT.rglob("*.log") if p.is_file()]
...
def validate_compileall(*, strict: bool = True) -> None:
    ...
    skip_parts = {".git", ".venv", "venv", "__pycache__", ".mypy_cache", ".pytest_cache", "build", "dist", ".tox"}
    errors: list[str] = []
    for path in PROJECT_ROOT.rglob("*.py"):
        if set(path.relative_to(PROJECT_ROOT).parts) & skip_parts:
            continue
