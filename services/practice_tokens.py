...
def get_wallet(user_id: int) -> PracticeWallet:
    try:
        with db() as conn:
            ...
            row = conn.execute("SELECT user_id, available_tokens, reserved_tokens, used_tokens FROM practice_wallets WHERE user_id=?", (int(user_id),)).fetchone()
    except Exception:  # validator: allow-wide-except
        return PracticeWallet(int(user_id), 0, 0, 0)
    return PracticeWallet(int(row["user_id"]), int(row["available_tokens"]), int(row["reserved_tokens"]), int(row["used_tokens"]))
...
def grant_tokens(user_id: int, *, package_id: str, amount: int, provider: str = "manual", provider_payment_id: str = "", source: str = "", idempotency_key: str | None = None) -> tuple[bool, PracticeWallet, int | None]:
    ...
    return True, get_wallet(int(user_id)), ledger_id
...
def grant_tokens_for_payment(*, provider: str, provider_payment_id: str, user_id: int, package_id: str, source: str = "webhook") -> tuple[bool, PracticeWallet, int | None]:
    ...
    with db() as conn:
        with tx(conn):
            ...
            if existing:
                return False, get_wallet(int(user_id)), int(existing["ledger_id"] or 0)
