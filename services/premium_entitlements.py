...
def consultation_admin_note(*, user_id: int, platform: str, external_user_id: str | None, package_title: str, provider_payment_id: str) -> str:
    return (
        ...
        "Admin must contact the user and schedule a 60-minute consultation."
    )


def video_course_message(*, package_title: str) -> str:
    return (
        "Video course access is open\n\n"
        f"Package: {package_title}.\n\n"
        "Video course URL:\n"
        f"{video_course_url()}\n\n"
        "Use this at a comfortable pace. This is not medical care or psychotherapy."
    )
...
def consultation_user_message(*, package_title: str) -> str:
    return (
        ...
        f"Package: {package_title}.\n"
        "The package includes one 60-minute consultation request.\n\n"
        "An administrator will contact you in this messenger."
    )
...
def _insert_entitlement(
    ...
    *,
    ...
    cursor = conn.execute(
        ...
        """.strip(),
        (
            int(user_id), package_id, entitlement_type, provider, provider_payment_id,
            source, "active", f"premium:{provider}:{provider_payment_id}:{entitlement_type}",
        ),
    )
...
def _insert_consultation_request(
    conn: Any,
    *,
    user_id: int,
    platform: str,
    ...
    cursor = conn.execute(
        ...
        """.strip(),
        (
            int(user_id), normalize_platform(platform), external_user_id, package_id, provider,
            provider_payment_id, "new", note, note, f"consultation:{provider}:{provider_payment_id}",
        ),
    )
    return int(getattr(cursor, "rowcount", 0) or 0) > 0


def grant_premium_entitlements_for_payment(
    *,
    user_id: int,
    package_id: str,
    provider: str,
        ...
        with tx(conn):
            ...
            if VIDEO_ENTITLEMENT in entitlement_types:
                ...
                for platform, external_user_id in targets:
                    if _insert_outbox(
                        ...
                        body=video_course_message(package_title=package.title),
                        idempotency_key=f"premium_delivery:{provider}:{provider_payment_id}:video:{platform}:{external_user_id or ''}",
                    ):
                        outbox_created += 1
            ...
            if CONSULTATION_ENTITLEMENT in entitlement_types:
                ...
                if _insert_outbox(
                    ...
                    body=consultation_user_message(package_title=package.title),
                    idempotency_key=(
                        f"premium_delivery:{provider}:{provider_payment_id}:"
                        f"consultation_user:{primary_platform}:{primary_external_user_id or ''}"
                    ),
                ):
                    outbox_created += 1
    ...
    return PremiumGrantResult(
        video_granted=video_granted,
        consultation_granted=consultation_granted,
...
def pending_delivery(limit: int = 20, *, user_id: int | None = None) -> list[dict[str, Any]]:
    with db() as conn:
        ...
        if user_id is None:
            rows = conn.execute(
                """
                SELECT id, user_id, platform, external_user_id, delivery_kind, title, body, attempts, last_error
...
def mark_delivery_sent(delivery_id: int) -> None:
    with db() as conn:
        with tx(conn):
            ensure_schema(conn)
            conn.execute(
                "UPDATE premium_delivery_outbox SET status='sent', updated_at=CURRENT_TIMESTAMP WHERE id=?",
                (int(delivery_id),),
            )
...
def consultation_requests_summary(limit: int = 20, *, user_id: int | None = None) -> list[dict[str, Any]]:
    with db() as conn:
        ...
        if user_id is None:
            rows = conn.execute(
                """
                SELECT id, user_id, platform, external_user_id, package_id, provider_payment_id,
                       status, contact_payload, admin_note, created_at
