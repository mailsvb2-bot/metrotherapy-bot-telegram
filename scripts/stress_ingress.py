...
This tests nginx/aiohttp ingress availability, latency distribution and status
stability without spamming Telegram/VK/MAX provider APIs.
"""
...
def _request_sync(*, target: str, method: str, url: str, body: bytes | None = None, headers: dict[str, str] | None = None, expect_status: set[int] | None = None) -> ProbeResult:
    ...
    try:
        ...
        return ProbeResult(target=target, ok=False, status=0, latency_ms=round(latency, 2), detail=f"{type(exc).__name__}: {exc}")
    latency = (time.perf_counter() - started) * 1000.0
...
async def run(args: argparse.Namespace) -> int:
    ...
    local_health = args.local_health_url
    local_webhook_health = args.local_webhook_health_url
    sem = asyncio.Semaphore(args.concurrency)
    tasks: list[asyncio.Task[ProbeResult]] = []

    headers = {"Content-Type": "application/json"}
    for i in range(args.requests):
        tasks.append(asyncio.create_task(_bounded_probe(sem, target="local_health", method="GET", url=local_health, expect_status={200})))
        tasks.append(asyncio.create_task(_bounded_probe(sem, target="local_webhook_health", method="GET", url=local_webhook_health, expect_status={200})))
        tasks.append(asyncio.create_task(_bounded_probe(sem, target="public_health", method="GET", url=f"{base}/healthz", expect_status={200})))
        tasks.append(asyncio.create_task(_bounded_probe(sem, target="vk_ignored_post", method="POST", url=f"{base}/webhooks/vk", body=_vk_ignored_payload(i), headers=headers, expect_status={200})))
        tasks.append(asyncio.create_task(_bounded_probe(sem, target="max_ignored_post", method="POST", url=f"{base}/webhooks/max", body=_max_ignored_payload(i), headers=headers, expect_status={200})))
        if args.include_invalid_json:
            tasks.append(asyncio.create_task(_bounded_probe(sem, target="vk_invalid_json", method="POST", url=f"{base}/webhooks/vk", body=b"{bad-json", headers=headers, expect_status={400})))
            tasks.append(asyncio.create_task(_bounded_probe(sem, target="max_invalid_json", method="POST", url=f"{base}/webhooks/max", body=b"{bad-json", headers=headers, expect_status={400})))
