...
"""Hermetic deep user-journey probe for token, demo and messenger boundaries."""
    ...
    try:
        try:
            ...
            for candidate in (uid, concurrency_uid, race_uid):
                _ensure_user(deps["db"], candidate)

            full_first = deps["pick_for_slot"]("morning", 0)
            full_second = deps["pick_for_slot"]("evening", 0)
            ...
            same_session = deps["create_session"](
                concurrency_uid,
                kind="work",
                source="settings",
                day="2026-07-12",
            ...
            same_a, same_b = _concurrent_two(
                lambda: deps["reserve_practice"](
                    concurrency_uid,
                    session_id=int(same_session),
                    audio_anchor=int(full_first.anchor),
                ),
                lambda: deps["reserve_practice"](
                    concurrency_uid,
                    session_id=int(same_session),
            ...
            same_wallet = deps["get_wallet"](concurrency_uid)
            ...
            if same_ids:
                deps["release_reservation"](next(iter(same_ids)), reason="deep_probe_reset")
            ...
            race_session_a = deps["create_session"](
                race_uid,
                kind="work",
                source="settings",
                day="2026-07-12",
            ...
            race_session_b = deps["create_session"](
                race_uid,
                kind="home",
                source="settings",
                day="2026-07-12",
            ...
            race_a, race_b = _concurrent_two(
                lambda: deps["reserve_practice"](
                    race_uid,
                    session_id=int(race_session_a),
                    audio_anchor=int(full_first.anchor),
                ),
                lambda: deps["reserve_practice"](
                    race_uid,
                    session_id=int(race_session_b),
                    audio_anchor=int(full_second.anchor),
                ),
            )
            race_successes = sum(1 for result in (race_a, race_b) if result[0])
            race_wallet = deps["get_wallet"](race_uid)
            ...
            deps["set_user_timezone"](uid, "Europe/Amsterdam")
            start = deps["start_or_resume_paid_practice"](uid)
            ...
            deps["fast_send_audio"].send_audio_cached = fail_send
            wallet_before_fail = deps["get_wallet"](uid)
            send_failed = False
            try:
                asyncio.run(
                    deps["complete_pre_score_and_send"](
                        uid,
                        platform="telegram",
                        score=2,
                        senders=deps["SenderRegistry"](),
                        telegram_bot=object(),
                ...
                send_failed = True
            wallet_after_fail = deps["get_wallet"](uid)
            failed_session = deps["get_session"](int(start.session_id))
            ...
            deps["fast_send_audio"].send_audio_cached = success_send
            ...
            wallet_after_send = deps["get_wallet"](uid)
            ...
            wallet_after_duplicate = deps["get_wallet"](uid)
            ...
            confirmed = deps["confirm_pending_audio_delivery"](
                uid,
                platform="telegram",
            ...
            post_result = asyncio.run(
                deps["complete_post_score_and_send_next"](
                    uid,
                    platform="telegram",
                    score=5,
                    senders=deps["SenderRegistry"](),
            ...
            final_first_session = deps["get_session"](int(start.session_id))
            _record(checks, problems, "post_score_saved", post_result.ok and int(final_first_session.post_score) == 5)
            _record(
                checks,
                problems,
                "stale_post_callback_does_not_overwrite",
                duplicate_post.transport == "post_score_already_saved" and int(final_first_session.post_score) == 5,
            )

            second_start = deps["start_or_resume_paid_practice"](uid)
            ...
            wallet_before_replay = deps["get_wallet"](uid)
            replay_result = asyncio.run(
                deps["send_next_audio_to_user"](
                    uid,
                    senders=deps["SenderRegistry"](),
                    telegram_bot=object(),
            ...
            wallet_after_replay = deps["get_wallet"](uid)
            ...
            _record(checks, problems, "wallet_unlocks_legacy_active_check", deps["is_active"](uid))
            _record(checks, problems, "wallet_unlocks_morning_settings", deps["has_access"](uid, "morning"))
            _record(checks, problems, "wallet_unlocks_evening_settings", deps["has_access"](uid, "evening"))
            _record(checks, problems, "wallet_unlocks_both_scope", deps["has_access"](uid, "both"))
            _record(checks, problems, "paid_wallet_suppresses_sales", deps["should_skip_sales"](uid))
            _record(checks, problems, "wallet_user_is_auto_audio_eligible", uid in deps["eligible_user_ids"]("morning"))

            canonical_demo_uid, demo_replies = deps["handle_incoming_text"](
                demo_external_uid,
                platform="max",
            ...
            full_before_demo = deps["get_progress_snapshot"](canonical_demo_uid)
            ...
            full_after_demo_send = deps["get_progress_snapshot"](canonical_demo_uid)
            deps["confirm_pending_audio_delivery"](
                canonical_demo_uid,
                platform="max",
                sequence_key="demo",
            )
            full_after_demo_done = deps["get_progress_snapshot"](canonical_demo_uid)
            ...
            _record(checks, problems, "demo_delivery_is_free", deps["get_wallet"](canonical_demo_uid).used_tokens == 0)
            _record(checks, problems, "demo_send_succeeds", demo_result.ok and demo_post.ok)
            _record(checks, problems, "demo_usage_is_recorded", "work" in deps["demo_sent_kinds"](canonical_demo_uid))
            _record(
                checks,
                problems,
            ...
            _, repeat_replies = deps["handle_incoming_text"](
                demo_external_uid,
                platform="max",
                external_user_id=str(demo_external_uid),
                text="demo_work",
            ...
            detail.update(
                {
                    "paid_wallet": asdict(deps["get_wallet"](uid)),
                    "concurrency_wallet": asdict(deps["get_wallet"](concurrency_uid)),
                    "race_wallet": asdict(deps["get_wallet"](race_uid)),
